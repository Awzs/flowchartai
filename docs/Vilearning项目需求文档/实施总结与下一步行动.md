...
> 【2025-11-02 更新】思维导图模式已在侧栏上线：`/api/ai/chat/mindmap` 支持文本生成，DisplayRegistry 解析并渲染 MindElixir 预览，并通过新建的 `mindmaps` 表持久化结果（暂未回写 Excalidraw 画布，extend 差分仍在规划）。  
> 【2025-11-02 更新】准备期文档资产（PoC 评估、DisplayRegistry 使用说明、上下文契约草案、质量保障预案、数据迁移草案）已补齐，迁移脚本雏形 `scripts/migrations/draft-boards-displays.ts` 可 dry-run 输出建表/迁移 SQL；`boards/displays/contexts` 表仍待实施。  
> 【2025-11-02 更新】DisplayRegistry 与思维导图 Display PoC 已完成并接入消息流；新添 `mindmap-converter` 单测与 mindmap 计费埋点的 `latencyMs` 指标，但端到端测试与监控告警尚未覆盖多 Display 场景。阶段 0 启动前需进一步补齐 Canvas 写回、extend 合并与监控闭环。
